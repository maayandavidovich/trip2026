...
(function(){
  ...
  function update(){
    ...
    var main=document.getElementById('booking-summary-main'); if(main) main.textContent=done+' סגור'+(done===1?'':'ים')+' · '+(boxes.length-done)+' עדיין פתוח'+(boxes.length-done===1?'':'ים'); var tc=document.getElementById('next-action-title-card'), nc=document.getElementById('next-action-note-card'); if(firstUrgent){var item=firstUrgent.closest('.check-item');var name=item.querySelector('.check-name');var label=name ? name.childNodes[0].textContent.trim() : 'משימה פתוחה'; var detail=name ? name.querySelector('small') : null; var detailText=detail ? detail.textContent.trim() : 'פתחי את ההזמנות כדי לראות בדיוק מה חסר.'; if(t)t.textContent=label; if(n)n.textContent=detailText; if(tc)tc.textContent=label; if(nc)nc.textContent=detailText;}else{var doneText='כל ההזמנות הדחופות הושלמו';if(t)t.textContent=doneText;if(n)n.textContent='השלב הבא הוא לעבור על ההזמנות בעדיפות גבוהה.';if(tc)tc.textContent=doneText;if(nc)nc.textContent='אין כרגע משימת הזמנה דחופה.';}
  }
  update();
})();
